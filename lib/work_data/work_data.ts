  ...
  // ─── PROJECT 3 ───────────────────────────────────────────────────────────────
  {
    ...
    deepDive: {
      ...
      processSteps: {
        ...
        steps: [
          { title: "API contract & infrastructure setup", description: "Defined the full REST API contract in week one. Provisioned AWS ECS, S3, CloudFront, and RDS so both teams could work independently against mocked endpoints." },
          { title: "NestJS microservices & Stripe billing", description: "Built auth service (JWT + refresh rotation), content service (S3 signed URLs + CloudFront), and billing service (Stripe webhooks for subscription lifecycle). Each is a separate NestJS module." },
          { title: "Flutter app — streaming & downloads", description: "Built the app with video_player for streaming, background_downloader for offline content, and Riverpod for state management. Adaptive quality selection based on network conditions." },
          { title: "Load testing & App Store submission", description: "Ran k6 load tests simulating 10k concurrent users. Tuned CloudFront cache policies and ECS autoscaling. Submitted to App Store and Play Store in week nine, approved in week ten." },
        ],
      },
      architecture: {
        heading: "System Architecture",
        ...
        layers: [
          { name: "API Gateway", tech: "AWS API Gateway", description: "Single entry point for all Flutter client requests. Routes to auth, content, or billing service. Handles rate limiting and request validation." },
          { name: "Auth Service", tech: "NestJS + JWT + Redis", description: "Issues short-lived access tokens (15 min) and long-lived refresh tokens stored in Redis. Silent refresh means users are never prompted to log in again." },
          { name: "Content Service", tech: "NestJS + S3 + CloudFront", description: "Manages the content catalogue and generates signed CloudFront URLs for authorised playback. Signed URLs expire after 6 hours." },
          { name: "Billing Service", tech: "NestJS + Stripe", description: "Wraps Stripe's subscription API. Handles payment_failed, subscription_cancelled, and invoice_paid webhook events and updates the user's access tier accordingly." },
          ...
          { name: "Mobile App", tech: "Flutter + Riverpod", description: "Single codebase for iOS and Android. Riverpod providers manage auth state, content catalogue, and download queue. Offline downloads stored in encrypted local storage." },
        ],
      },
      techCategories: [
      ...
      approach: {
        heading: "How We Approached the Problem",
        body: "The biggest risk was timeline — 10 weeks is tight for a full streaming platform. We managed it by agreeing on the API contract before writing any code, letting the Flutter team work against a mock server while NestJS services were built. The second risk was content security — signed CloudFront URLs with short TTLs let us ship without DRM.",
        rejected: [
          { option: "Single monolithic NestJS app", reason: "Content delivery and billing have very different scaling profiles. Separating them meant we could scale the content service independently during peak hours." },
          ...
          { option: "Self-hosted video transcoding", reason: "Building a transcoding pipeline would have added 3-4 weeks. We used a third-party service to convert uploads to HLS and handed the output directly to S3." },
        ],
      },
      dataModel: {
        heading: "Data Modelling",
